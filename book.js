...
function get_code_block_buttons(block, pre_block) {
  ...
  if (!buttons) {
    ...
    Array.from(block.classList).forEach(function (className) {
      var myRegexp = /^language-(.+)$/g;
      var match = myRegexp.exec(className);
      if (match != null) {
        var languageName = hljs?.getLanguage(match[1])?.name;

        if (languageName) {
          codeLanguage.innerText = languageName;
        } else {
          codeHeader.classList.add('unknow');
          codeLanguage.innerText = match[1];
        }
      }

      if (className != 'hljs') {
        codeHeader.classList.add(className);
      }
    });

    codeHeader.appendChild(codeLanguage);
    codeHeader.appendChild(buttons);
    pre_block.insertBefore(codeHeader, pre_block.firstChild);
  }
...
(function codeSnippets() {
  ...
  Array.from(document.querySelectorAll('code.language-rust')).forEach(function (
    ...
    var buttons = get_code_block_buttons(block, pre_block);

    var showHiddenLinesButton = document.createElement('button');
    showHiddenLinesButton.className = 'fa fa-eye';
    showHiddenLinesButton.title = 'Show hidden lines';
    showHiddenLinesButton.setAttribute('aria-label', 'Show hidden lines');

    buttons.appendChild(showHiddenLinesButton);
  ...
  Array.from(document.querySelectorAll('.playground')).forEach(function (
    ...
    // Add play button
    var buttons = get_code_block_buttons(code_block, pre_block);
